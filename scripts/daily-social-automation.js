...
function buildCacheBustedImageUrl(puzzleDate) {
  // Handoff spec: ?v=YYYY-MM-DD (puzzle date, not a UTC date).
  return `${IMAGE_URL}?v=${puzzleDate}`;
}

function buildPostCopy(puzzle, puzzleNumber) {
  const { A, B, C } = puzzle.easy.labels;
  return `Puzzle #${puzzleNumber} — Most people can't solve this in 3 tries.\n\nWhat do ${A}, ${B} and ${C} all have in common?\n\nThink you can find the Common Ground? commongroundpuzzle.com`;
}

async function updateGoogleSheet(puzzle, puzzleNumber, imageUrl) {
  if (!SHEET_ID) {
    console.log("GOOGLE_SHEETS_ID not set; skipping Google Sheet update.");
    return;
  }
  if (!SERVICE_ACCOUNT_JSON) {
    console.log("GOOGLE_SERVICE_ACCOUNT not set; skipping Google Sheet update.");
    return;
  }
  ...
  const postCopy = buildPostCopy(puzzle, puzzleNumber);
  ...
  await sheets.spreadsheets.values.append({
    ...
    requestBody: {
      values: [[
        puzzle.date,
        puzzleNumber,
        puzzle.easy.labels.A,
        puzzle.easy.labels.B,
        puzzle.easy.labels.C,
        postCopy,
        imageUrl
      ]]
    }
...
async function main() {
  ...
  const imageUrl = buildCacheBustedImageUrl(puzzle.date);
  ...
  await updateGoogleSheet(puzzle, puzzleNumber, imageUrl);
  console.log(`Image URL written to sheet: ${imageUrl}`);
}
